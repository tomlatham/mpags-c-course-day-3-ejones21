			...
			ProgramSettings& thecommands)
{
  ...
  for (size_type i {1}; i < nArgs; ++i) {
    ...
    if (args[i] == "-h" || args[i] == "--help") {
      ...
      thecommands.helpRequested = true;
      break;
    }
    else if (args[i] == "--version") {
      // Set the indicator and terminate the loop
      thecommands.versionRequested = true;
      break;
    }
    else if (args[i] == "-i") {
      ...
      if (i == nArgs-1) {
        ...
        thecommands.inputFile = args[i+1];
        ++i;
      }
    }
    else if (args[i] == "-o") {
      ...
      if (i == nArgs-1) {
        ...
        // Got filename, so assign value and advance past it
        thecommands.outputFile = args[i+1];
        ++i;
      }
    }
    else if (args[i] == "-k") {
      ...
      if (i == nArgs-1) {
        ...
        thecommands.cipher_key = args[i+1];
        ++i;
      }
    }
    else if ( args[i] == "--encrypt" ) {
      thecommands.mode = CipherMode::encrypt;
    }
    else if ( args[i] == "--decrypt" ) {
      thecommands.mode = CipherMode::decrypt;
    }
    else {
